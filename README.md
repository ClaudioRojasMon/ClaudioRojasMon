
![Analítica](Logo-completo.png)

# 👋 Hola, soy Claudio Rojas

## Director Académico | Magíster en Ciencia de Datos

Llevo 20 años en educación, 10 como Director Académico. Estudié ciencia de datos para resolver problemas reales que veía en mi trabajo: decisiones sin datos, reportes que tomaban horas, falta de herramientas adaptadas al contexto chileno.

**Mi enfoque:** Identifico problemas educativos reales, diseño soluciones prácticas, y las implemento usando Python, R e IA. No soy programador de profesión, pero sí alguien que sabe usar estas herramientas para crear impacto medible en instituciones educativas.

---

## 📊 Resultados recientes

- **Sistema de Business Intelligence** que redujo 80% el tiempo de reportería para 1,400+ estudiantes
- **Marco institucional de IA generativa** adoptado oficialmente por 100+ docentes
- **Automatización de planificación docente** que redujo 67% el tiempo en tareas repetitivas
- **2 libros interactivos de Python** publicados en [galería internacional de Executable Books](https://executablebooks.org/en/latest/gallery/)

---

## 🚀 Proyectos destacados

Cada proyecto resuelve un problema educativo real con resultados medibles:

| Proyecto | Problema que resuelve | Impacto |
|----------|----------------------|---------|
| [📖 analizador-lexile-chile](https://github.com/ClaudioRojasMon/analizador-lexile-chile) | Herramientas Lexile caras y en inglés | Análisis gratuito en español adaptado a Chile |
| [📊 paes-ranking-chile](https://github.com/ClaudioRojasMon/paes-ranking-chile) | Falta de análisis accesible de resultados PAES | Sistema completo de ranking y análisis |
| [📈 Trayectorias_Academicas](https://github.com/ClaudioRojasMon/Trayectorias_Academicas) | Sin seguimiento longitudinal de estudiantes | Análisis de trayectorias 2017-2020 con R |
| [📗 Apoyo - Libro Interactivo](https://claudiorojasmon.github.io/Apoyo/intro.html) | Falta material Python para educación media | Jupyter Book con Python básico, datos y NLP |
| [📘 Programando la Historia](https://claudiorojasmon.github.io/apuntes_2025/intro.html) | Enseñar IA de forma crítica a adolescentes | Libro que usa historia para enseñar Python e IA |

---

## 🛠️ Herramientas que uso

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat&logo=r&logoColor=white)
![Jupyter](https://img.shields.io/badge/-Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)
![Pandas](https://img.shields.io/badge/-Pandas-150458?style=flat&logo=pandas&logoColor=white)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white)

**Stack técnico:**
- **Lenguajes:** Python (pandas, numpy, scikit-learn, matplotlib), R (ggplot2, tidyverse)
- **IA generativa:** Claude, ChatGPT, Gemini - para automatización y solución de problemas
- **Business Intelligence:** Looker Studio, dashboards interactivos
- **Desarrollo:** Jupyter Notebook/Book, Git/GitHub
- **Machine Learning:** K-means clustering, análisis predictivo

**Filosofía de trabajo:** Uso las herramientas más efectivas disponibles (incluyendo IA) para resolver problemas. Lo importante es el impacto medible, no si escribí cada línea de código manualmente.

---

## 💼 Áreas de trabajo

- 📊 Learning Analytics y Business Intelligence educativo
- 🤖 Integración de IA generativa en procesos pedagógicos
- 📈 Análisis de trayectorias académicas y predicción de riesgo
- 🎓 Transformación digital en instituciones educativas
- 📚 Creación de recursos educativos interactivos
- 🔍 Análisis de datos para políticas educativas

---

## 🎯 Qué busco

Estoy explorando oportunidades donde mi combinación única de experiencia educativa profunda y capacidad técnica genere valor:

- **Consultoría** en transformación digital educativa
- **EdTech** (Product Management, Implementation, Learning Analytics)
- **Innovación educativa** con enfoque en datos e IA
- **Liderazgo** en proyectos que combinen educación y tecnología

---

## 📚 Formación académica

- **Magíster en Ciencias de Datos para la Innovación** | Universidad de Concepción (2021-2023)
- **Magíster en Educación, mención Gestión y Liderazgo** | Pontificia Universidad Católica de Chile (2017-2018)
- **Licenciatura en Historia** | Pontificia Universidad Católica de Chile (1993-1997)

**Formación complementaria:**
- Diplomado en Ciencias de Datos para la Toma de Decisiones | UDP (2020-2021)
- Diplomado en Liderazgo Educacional | PUC (2016)
- People Analytics: Estrategia Basada en Datos | UAI (2025)

---

## 📫 Conectemos

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/claudio-rojas-monsalves)
[![Email](https://img.shields.io/badge/-Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:crojasmon@bitacoralab.cl)

---

💙 Desarrollado con pasión por la educación desde el sur de Chile 🇨🇱
